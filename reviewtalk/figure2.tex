...
%		edge[->,very thick]			node[auto,swap]{$ans_l$}	(q0);
	...
	\node 	(q4)		at	(3,-3) 	[circle, thick, draw=blue!50,fill=blue!20,inner sep=0pt,minimum size=.75cm]{$q_4$}
		edge[<-,very thick]			node[auto,swap]{$req_l$}	(q1);

	\node 	(q5)		at	(3,0) 	[circle, thick, draw=blue!50,fill=blue!20,inner sep=0pt,minimum size=.75cm]{$q_5$}
		edge[<-,very thick]			node[auto]{$ans_l$}	(q4)
		edge[->,very thick]			node[auto,swap]{$ans_l$}	(q0);
